...
#    `functionName` is a function's name in your module (e.g., `assistedMigrationBCEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Git/assistedMigrationBC/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("assistedMigrationBC")
  path <- list(modulePath = "C:/Ian/Git",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    assistedMigrationBC = list(.saveInitialTime = NA)
  )
  ...
  if (exists("assistedMigrationBCEvent1", envir = .GlobalEnv)) {
    simOutput <- assistedMigrationBCEvent1(mySim)
  } else {
  ...
  if (exists("assistedMigrationBCEvent2", envir = .GlobalEnv)) {
    simOutput <- assistedMigrationBCEvent2(mySim)
  } else {
